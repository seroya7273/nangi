...
if (!QA_FINAL_EXTERNAL_USERS) {
	...
	if (qa_clicked('dowallpost')) {
		...
		if (!strlen($inmessage)) {
			...
			qa_wall_add_post($loginuserid, qa_get_logged_in_handle(), qa_cookie_get(), $userid, $useraccount['handle'], $inmessage . qa_lang('profile/post_wall_amount_withdrawn'), '');
			...
			qa_db_deduct_coins($userid, $inmessage);
...
$qa_content['form_activity'] = array(
	...
	'fields' => array(
		...
		'points' => array(
			...
			'value' => (qa_db_get_coins_balance($userid) == 1)		 : 16/01/22. To display actual total of coins
				? qa_lang_html_sub('main/1_point', '<span class="qa-uf-user-points">1</span>', '1')
				: qa_lang_html_sub('main/x_points', '<span class="qa-uf-user-points">' . qa_html(qa_format_number(qa_db_get_coins_balance($userid))) . '</span>'),
...
// Wall posts
/* Mel: 16/01/22. Hide entire withdrawal wall from this page.
