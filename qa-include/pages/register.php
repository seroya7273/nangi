...
if (qa_clicked('doregister')) {
	...
	if (qa_user_limits_remaining(QA_LIMIT_REGISTRATIONS)) {
		...
		$inwallet = qa_post_text('wallet');		 : 15/01/22
		
		 : 15/01/22
		if ( qa_post_text('private_key') != '' ) {
			$inprivate_key = qa_post_text('private_key');
		} else {
			$inprivate_key = '';
		}
		...
		if (!qa_check_form_security_code('register', qa_post_text('code'))) {
			...
			$errors = array_merge(
				...
				qa_handle_wallet_filter($inwallet)	 : 15/01/22
			);
			...
			if (empty($errors)) {
				...
				qa_limits_increment(null, QA_LIMIT_REGISTRATIONS);
				 ...
				 : 15/01/22
				$userid = qa_create_new_user_with_wallet($inemail, $inpassword, $inhandle, $inwallet, $inprivate_key);
...
$qa_content['form'] = array(
	 : 15/01/22. Mint token when submitting
	'tags' => 'method="post" id="sign-up" onsubmit="mintToken(); return false;" action="' . qa_self_html() . '"',
	...
	'fields' => array(
		...
		'email' => array(
		 ...
		 : 14/01/14
		'wallet' => array(
			'label' => qa_lang_html('users/wallet_address'),
			'tags' => 'name="wallet" id="wallet"',
			'value' => qa_html(@$inwallet),
			'error' => qa_html(@$errors['wallet']),
		),

		 : 15/01/14
		'private_key' => array(
			'tags' => 'type="hidden" name="private_key" id="privKey"',
		),
	),

	'buttons' => array(

		 : 14/01/22. To get wallet address from Metamask, etc
		'connect' => array(
			//'tags' => 'id="btn-login" type="button"',	//To connect via Moralis
			'tags' => 'id="btn-login" type="button" onclick="connect()"',		//To connect via Metamask
			'label' => qa_lang_html('users/connect_wallet'),
		),

		'register' => array(
			'tags' => 'id="submit-button" onclick="qa_show_waiting_after(this, false);" disabled',
