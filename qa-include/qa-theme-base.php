	...
	public function message_content($message)
	{
		if (!empty($message['content'])) {
			$this->output('<div class="qa-message-content">');
			 ...
			 : 16/01/22. Convert URL contained in withdrawal message to a clickable URL
			$url = '@(http(s)?)?(://)?(([a-zA-Z])([-\w]+\.)+([^\s\.]+[^\s]*)+[^,.\s])@';
			$message = preg_replace($url, '<a href="http$2://$4" target="_blank" title="' . qa_lang('profile/tx_receipt') . '">' . qa_lang('profile/tx_receipt') . '</a>', ($message['content']));
