...
class qa_filter_basic
{
	...
	public function filter_handle(&$handle, $olduser)
	{
	 ...
	 : 15/01/22. To check wallet field is not empty
	public function filter_wallet(&$wallet, $olduser)
	{
		if (!strlen($wallet)) {
			return qa_lang('users/wallet_empty');
		}
	}
	...
	/** Mel: 16/01/22
	 * Check if allocated coins meet the min requirement.
	 *
	 * 
	 */
	public function validate_min_coins(&$errors, $field, $input, $minamount)
	{
		if ($input < $minamount) {
			$errors['extra2'] = qa_lang_sub('main/min_amount_x', $minamount);
		}
	}

	/** Mel: 19/01/22
	 * Check if requester has enough coins to reward for posting.
	 *
	 * 
	 */
	public function validate_coins_balance(&$errors, $userid, $input)
	{

		$coinsbalance = qa_db_get_coins_balance($userid);

		if ($input > $coinsbalance) {
			$errors['extra2'] = qa_lang_sub('main/insufficient_coins', $coinsbalance);
		}
	}
