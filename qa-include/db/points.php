...
if (!defined('QA_VERSION')) { // don't allow this page to be requested directly from browser
 ...
 : 14/01/22. To call the qa_db_usermeta_get function
...
function qa_db_points_update_ifuser($userid, $columns)
{
	...
	if (qa_should_update_counts() && isset($userid)) {
		...
		$query = 'INSERT INTO ^userpoints (' . $insertfields . 'points) VALUES (' . $insertvalues . $insertpoints . ') ' .
			//'ON DUPLICATE KEY UPDATE ' . $updates . 'points=' . $updatepoints . '+bonus + coins';  : 14/01/22. Add coins into calculation
 ...
 : 14/01/22
/**
 * To add reward points to the one who's answer is selected.
 * @param $userid
 * @param $columns
 * @return mixed
 */
function qa_db_coins_add_ifuser($userid, $columns, $post_id, $extra_field)
{
	if (qa_to_override(__FUNCTION__)) { $args=func_get_args(); return qa_call_override(__FUNCTION__, $args); }

	//Get the coins set by questioner in the extra field on the question form 
	$coins = qa_db_postmeta_get($post_id, $extra_field);

	//DEBUG
	error_log(" Coins: " . $coins . " extra_field: " . $extra_field . " postid: " . $post_id . " userid: " . $userid, 0);
	...
	if (qa_should_update_counts() && isset($userid)) {
		...
		//DEBUG
		error_log(" Insertpoints: " . $insertpoints, 0);
		//DEBUG
		error_log(" Updatepoints: " . $updatepoints, 0);
		
		//Add coins into the userpoints table
		qa_db_give_coins($userid, $coins);
 ...
 : 14/01/22
/**
 * To deduct coins when questioner selects an answer. Their coins shall be given to answerer
 * @param $userid
 * @param $columns
 * @return mixed
 */
function qa_db_coins_deduct_ifuser($userid, $columns, $post_id, $extra_field)
{
	if (qa_to_override(__FUNCTION__)) { $args=func_get_args(); return qa_call_override(__FUNCTION__, $args); }
	...
	if (qa_should_update_counts() && isset($userid)) {
		...
		//Deduct coins at the userpoints table
		qa_db_deduct_coins($userid, $coins);
...
function qa_db_points_set_bonus($userid, $bonus)
{
 ...
 : 14/01/22
/**
 * Reward coins to $userid
 * @param $userid
 * @param $coins
 */
function qa_db_give_coins($userid, $coins)
{
	qa_db_query_sub(
		"INSERT INTO ^userpoints (userid, coins) VALUES ($, #) ON DUPLICATE KEY UPDATE coins = coins + #",
		$userid, $coins, $coins
	);
}
 ...
 : 14/01/22
/**
 * Deduct coins from $userid
 * @param $userid
 * @param $coins
 */
function qa_db_deduct_coins($userid, $coins)
{
	qa_db_query_sub(
		"INSERT INTO ^userpoints (userid, coins) VALUES ($, #) ON DUPLICATE KEY UPDATE coins = coins - #",
		$userid, $coins, $coins
 ...
 : 15/01/22
/**
 * Get coins balance from $userid. Total coins is sum of points and coins since coins column is the coeeficient.
 * @param $userid
 */
function qa_db_get_coins_balance($userid)
{
	return qa_db_read_one_value(qa_db_query_sub(
		"SELECT points+coins FROM ^userpoints WHERE userid=#",
		$userid
	));
}
 ...
 * @param $userid
 */
function qa_db_get_wallet_address($userid)
{
	return qa_db_read_one_value(qa_db_query_sub(
 ...
 : 16/01/22
/**
 * Get max amount user can withdraw
 * @param $userid
 * @
 */
function qa_db_get_max_withdrawal($userid)
{
	$userbalance = (int) qa_db_get_coins_balance($userid);
	$minbalance = (int) qa_opt('points_base');
	$maxamount = $userbalance - $minbalance;

	//DEBUG
	error_log("max amount: " . $maxamount);
