...
/** Mel: 15/01/22
 * Create a new user in the database with $email, $password, $handle, privilege $level, $ip address, $wallet and $private_key
 * @param $email
 * @param $password
 * @param $handle
 * @param $level
 ...
 * @return mixed
 */
function qa_db_user_create_with_wallet($email, $password, $handle, $level, $ip, $wallet, $private_key)
{
	require_once QA_INCLUDE_DIR . 'util/string.php';
	...
	if (QA_PASSWORD_HASH) {
		qa_db_query_sub(
			'INSERT INTO ^users (created, createip, email, passhash, level, handle, loggedin, loginip, wallet, privatekey) ' .
			'VALUES (NOW(), UNHEX($), $, $, #, $, NOW(), UNHEX($), $, $)',
			$ipHex, $email, isset($password) ? password_hash($password, PASSWORD_BCRYPT) : null, (int)$level, $handle, $ipHex, $wallet, $private_key
		);
	} else {
		$salt = isset($password) ? qa_random_alphanum(16) : null;

		qa_db_query_sub(
			'INSERT INTO ^users (created, createip, email, passsalt, passcheck, level, handle, loggedin, loginip, wallet, privatekey) ' .
			'VALUES (NOW(), UNHEX($), $, $, UNHEX($), #, $, NOW(), UNHEX($), $, $)',
			$ipHex, $email, $salt, isset($password) ? qa_db_calc_passcheck($password, $salt) : null, (int)$level, $handle, $ipHex, $wallet, $private_key
		);
	}
