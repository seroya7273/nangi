...
/** Mel: 15/01/22
 * Return $errors fields for any invalid aspect of $wallet. Works by calling through
 * to all filter modules and also rejects existing values in database unless they belongs to $olduser (if set).
 * @param $wallet
 * @param $olduser
 * @return array
 */
function qa_handle_wallet_filter(&$wallet, $olduser = null)
{
	require_once QA_INCLUDE_DIR . 'db/users.php';
	require_once QA_INCLUDE_DIR . 'util/string.php';
	...
	// sanitize 4-byte Unicode and invisible characters
	$wallet = qa_remove_utf8mb4($wallet);
	$wallet = preg_replace('/\p{C}+/u', '', $wallet);

	$filtermodules = qa_load_modules_with('filter', 'filter_wallet');

	foreach ($filtermodules as $filtermodule) {
		$error = $filtermodule->filter_wallet($wallet, $olduser);
		if (isset($error)) {
			$errors['wallet'] = $error;
			break;
		}
	...
	//To be completed to check for existing wallet.
	// if (!isset($errors['handle'])) { // first test through filters, then check for duplicates here
	// 	$handleusers = qa_db_user_find_by_handle($handle);
	// 	if (count($handleusers) && ((!isset($olduser['userid'])) || (array_search($olduser['userid'], $handleusers) === false)))
	// 		$errors['handle'] = qa_lang('users/handle_exists');
...
/** Mel: 15/01/22
 * Create a new user (application level) with $email, $password, $handle, $level and $wallet.
 * Set $confirmed to true if the email address has been confirmed elsewhere.
 * Handles user points, notification and optional email confirmation.
 * @param $email
 * @param $password
 ...
 * @return mixed
 */
function qa_create_new_user_with_wallet($email, $password, $handle, $wallet, $private_key, $level = QA_USER_LEVEL_BASIC, $confirmed = false)
{
	if (qa_to_override(__FUNCTION__)) { $args=func_get_args(); return qa_call_override(__FUNCTION__, $args); }
	...
	$userid = qa_db_user_create_with_wallet($email, $password, $handle, $level, qa_remote_ip_address(), $wallet, $private_key);
	//$userid = qa_db_user_create($email, $password, $handle, $level, qa_remote_ip_address());
	qa_db_points_update_ifuser($userid, null);
	qa_db_uapprovecount_update();
