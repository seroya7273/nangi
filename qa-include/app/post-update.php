...
function qa_question_set_selchildid($userid, $handle, $cookieid, $oldquestion, $selchildid, $answers)
{
	...
	qa_db_points_update_ifuser($oldquestion['userid'], 'aselects');
	 ...
	 : 14/01/22. Deduct coins to questioner to be given to answerer
	qa_db_coins_deduct_ifuser($oldquestion['userid'], 'aselecteds', $oldquestion['postid'], 'qa_q_extra2');
	...
	if (isset($oldselchildid) && isset($answers[$oldselchildid])) {
		qa_db_points_update_ifuser($answers[$oldselchildid]['userid'], 'aselecteds');
		 ...
		 : 14/01/22. Give coins to answerer for getting their answer selected
		qa_db_coins_add_ifuser($answers[$oldselchildid]['userid'], 'aselecteds', $oldquestion['postid'], 'qa_q_extra2');
	...
	if (isset($selchildid)) {
		qa_db_points_update_ifuser($answers[$selchildid]['userid'], 'aselecteds');
		 ...
		 : 14/01/22. Give reward points to answerer
		qa_db_coins_add_ifuser($answers[$selchildid]['userid'], 'aselecteds', $oldquestion['postid'], 'qa_q_extra2');
