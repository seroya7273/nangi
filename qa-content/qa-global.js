	...
	if (elem && !elem.qa_waiting_shown) {
		...
		if (w) {
			...
			elem.qa_waiting_shown = c;
			 ...
			 : 16/01/22. Disable element like button to prevent clicking while waiting
			//elem.disabled = true;
...
function qa_hide_waiting(elem) {
	...
	if (c) {
		...
		elem.qa_waiting_shown = null;
		 ...
		 : 16/01/22. Turn back on element after waiting is over
		//elem.disabled = false;
...
async function qa_submit_wall_post(elem, morelink) {
	var params = {};
	...
	params.morelink = morelink ? 1 : 0;
	 ...
	 : 15/01/22
	params.balance = document.forms.wallpost.balance.value;			//The amount user has in account now
	params.minbalance = document.forms.wallpost.minbalance.value;	//Min amount required untouched in account
	params.wallet = document.forms.wallpost.wallet.value;

	amount_left = params.balance - params.message;

	//DEBUG
	console.log("Min balance requirement: " + params.minbalance);
	console.log("Amount left after withdrawal: " + amount_left);

	if (amount_left >= params.minbalance) {

		qa_show_waiting_after(elem, false);		//Display loading spinner gif

		await withdrawCoins(params.message, params.wallet).then(function (txhash) {
			params.txhashurl = qa_html_escape('https://testnet.snowtrace.io/tx/' + txhash + '');

			//DEBUG
			console.log("txhash: " + txhash);
			console.log("txhashurl: " + params.txhashurl);
		...
		alert("Exceeded max withdrawal amount. You must have at least " + params.minbalance + " left in your account after withdrawal");
	}
	 : End
	...
	window.location.reload();		 : 16/01/22. Refresh withdrawal page to get new balance after withdrawal.
