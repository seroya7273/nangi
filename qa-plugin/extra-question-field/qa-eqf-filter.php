		...
		for($key=1; $key<=qa_eqf::FIELD_COUNT_MAX; $key++) {
			if(qa_opt(qa_eqf::FIELD_ACTIVE.$key)) {
				...
				if(qa_opt(qa_eqf::FIELD_REQUIRED.$key)) {
					...
					$fb->validate_min_coins($errors, $name, $checkvalue, 100);	 : 16/01/22. To ensure min coins amount is met
					...
					$fb->validate_coins_balance($errors, $userid, $checkvalue);	//Check if requester has enough coins balance to ask for help
					 : End
